...
export default class Manage extends React.Component {
  ...
  onViewFather = async (comment) => {
    ...
    const content = await this.getFatherContent(comment.hostPage, comment.fatherID)
  ...
  getCardExtra = (comment) => {
    const viewSource = (<a key={'0'} target="_blank" href={this.getURLFromId(comment.hostPage)}>{this.getURLFromId(comment.hostPage)}</a>)
    let slashIndex = comment.commentId.indexOf('#')
    let defineChar = comment.commentId.charAt(slashIndex + 1)
    if (defineChar === 'S') {
      const viewFather = (<div key={'1'}><Button loading={this.state.fatherLoading} onClick={()=>this.onViewFather(comment)} type={'dashed'}>View Father</Button></div>)
  ...
  render() {
    return(
      <div style={{paddingTop: '50px'}}>
        <Title style={{fontFamily:'monospace', fontSize: '30px', textAlign: 'center'}}>Comments To Audit</Title>
        <Row>
          <Col span={3}/>
          <Col span={18}>
            <List
              ...
              renderItem={item => (
                <div style={{paddingBottom: '20px'}}>
                  <Card title={item.authorName + '  [' + item.email + ']'}  extra={this.getCardExtra(item)} actions={
                    [
                      <div><Button size={'small'} type={'primary'} onClick={()=>this.passComment(item)}>OK</Button></div>,
                      <div><Button size={'small'} type={'primary'} danger={true} onClick={()=>this.denyComment(item)}>Deny</Button> </div>
                    ]
                  }>
